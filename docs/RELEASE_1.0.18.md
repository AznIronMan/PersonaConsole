...
- Added `JOURNAL_FEATURE`, `JOURNAL_THEME_KEYS`,
  ...
  `render_journal_surface(...)` under both `persona_console` and
  `personacore`.
- Added `personacore.journal` as a public re-export module.
...
PersonaCore renders escaped, read-only journal pages. Consumers still own entry
...
PYTHONPATH=src python3 examples/fixture_app.py --output /tmp/personacore-fixture-1.0.18.html
