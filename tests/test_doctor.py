...
def test_consumer_integration_doctor_passes_current_source():
    report = run_consumer_integration_doctor(expected_version="1.0.17")
    data = report.as_dict()

    assert report.ok is True
    assert data["persona_console"]["version"] == "1.0.17"
    assert data["personacore"]["version"] == "1.0.17"
...
def test_consumer_integration_doctor_script_json():
    ...
    assert payload["persona_console"]["version"] == "1.0.17"
    assert payload["personacore"]["version"] == "1.0.17"
